...
algebra in which the values of the variables are true or false, typically
denoted with 1 or 0 respectively. Instead of elementary algebra where the
values of the variables are numbers and the main operations are addition
and multiplication, the main operations of Boolean algebra are the conjunction
(denoted ∧), the disjunction (denoted ∨) and the negation (denoted ¬).
...
def boolean(x, y, operation):
    if operation == OPERATION_NAMES[0]:
        ...
        return conjunction(disjunction(x,y), negation(conjunction(x,y)))

    return negation(conjunction(disjunction(x,y), negation(conjunction(x,y))))
